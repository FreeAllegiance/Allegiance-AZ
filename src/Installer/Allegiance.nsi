...
Section "${SHORT_PRODUCT_NAME} ${PRODUCT_VERSION} Game" SECgame
  ...
  DetailPrint "Checking Visual C++ 2015 Runtime installation..."
  Call VC2015Check
  DetailPrint ".... OK"
...
Section -AdditionalIcons
  ...
  CreateShortCut "$SMPROGRAMS\${SHORT_PRODUCT_NAME} ${PRODUCT_VERSION}\Allegiance-DX9.lnk" "$INSTDIR\Production\Allegiance-DX9.exe" "" "$INSTDIR\Production\Allegiance-DX9.exe" 0
  CreateShortCut "$SMPROGRAMS\${SHORT_PRODUCT_NAME} ${PRODUCT_VERSION}\Allegiance-DX7.lnk" "$INSTDIR\Production\Allegiance-DX7.exe" "" "$INSTDIR\Production\Allegiance-DX7.exe" 0
...
Section -Post
  ...
  ; ACSS registry keys - Note, ACSS depends on the 1.4 version of the registry. If you want to change
  ; this version number in the future, you will need to update ACSS and the Allegiance code base.
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "Lobby Path" "$INSTDIR"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4\" "ClientService" "https://acss.alleg.net/CSSServer/ClientService.svc"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4\" "ManagementWebRoot" "http://acss.alleg.net"
  

  ; ArtPath - Defines Artwork path
  WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "ArtPath" "$INSTDIR\Production\Artwork"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "BetaArtPath" "$INSTDIR\Beta\Artwork"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "ProductionArtPath" "$INSTDIR\Production\Artwork"
  
  ; EXE Path
  WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "EXE Path" "$INSTDIR\Production"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "Beta EXE Path" "$INSTDIR\Beta"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "Production EXE Path" "$INSTDIR\Production"
  
  ; CfgFile
  WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "CfgFile" "http://52.176.109.28/allegiance.txt"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "BetaCfgFile" "http://acss.alleg.net/allegiance-beta.txt"
  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "ProductionCfgFile" "http://acss.alleg.net/allegiance.txt"

  ;WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "SignupUrl" "http://acss.alleg.net/Login.aspx"
 ; WriteRegStr HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "ManagementWebRoot" "http://acss.alleg.net"
  
  ; FIRSTRUN
  WriteRegDWORD HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "FIRSTRUN" "1"
  ; HasTrained - Disabled
  WriteRegDWORD HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "HasTrained" "1"
  
  ; Graphic default settings: TexturePack disabled, Antialiasing 4x, MaxTextureSize 1024x1024
  WriteRegDWORD HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "UseTexturePack" "0"
  WriteRegDWORD HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "UseAntialiasing" "4"
  WriteRegDWORD HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4" "MaxTextureSize" "2"
  ...
  ${If} $1 == 1
    ...
    SimpleFC::AddApplication "${PRODUCT_NAME} ${PRODUCT_VERSION} - Game" "$INSTDIR\Production\Allegiance-DX9.exe" 0 2 "" 1
	SimpleFC::AddApplication "${PRODUCT_NAME} ${PRODUCT_VERSION} - Game" "$INSTDIR\Production\Allegiance-DX7.exe" 0 2 "" 1
...
Function .onInit
  ...
  StrCpy $url "https://github.com/FreeAllegiance/Artwork/releases/download/1.4.4"
...
Section Uninstall
 ...
 ${If} ${AtLeastWinVista}
   ExecWait "sdbinst -n $\"Free Allegiance 1.4 - Application Compatibility Database$\""
 ${EndIf}
  ...
  DeleteRegKey HKLM "Software\Microsoft\Microsoft Games\Allegiance\1.4"
  ...
  ${If} $1 == 1
    ; This runs only, if Windows Firewall is active
    SimpleFC::RemoveApplication "$INSTDIR\Production\Allegiance-DX9.exe"
	SimpleFC::RemoveApplication "$INSTDIR\Production\Allegiance-DX7.exe"
...
Function VC2015Check
	...
	; Fire up error, vc2015 is not installed
	MessageBox MB_ICONSTOP|MB_YESNO "$(^Name) couldn't detect Microsoft Visual C++ Runtimes v2015, would you like to install it now?" IDNO AbortVC2015
	
	SetDetailsPrint both
	DetailPrint "Starting Visual Studio 2015 redist setup... "
	SetDetailsPrint none
	; Run installer from temp folder
	; VC12 installer isn't bound at system language :)
	SetOutPath "$TEMP"
	File ".\Resources\VC15\vc_redist.x86.exe"
	...
	AbortVC2015:
	MessageBox MB_ICONSTOP|MB_OK "VC++ 2015 Runtimes are required to install this application. Setup will now exit."
	StrCpy $R9 "X"
	Call RelGotoPage ; Execution ends after this call (cancel button is virtually clicked)
